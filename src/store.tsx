...
function normalize(raw: Partial<AppData>): AppData {
  return { ...initial, ...raw, workouts: raw.workouts ?? [], exercises: raw.exercises ?? [], routines: raw.routines ?? [], settings: { ...initial.settings, ...raw.settings } };
}
...
export function StoreProvider({ children }: React.PropsWithChildren) {
  ...
  const api = useMemo<Store>(() => ({
    ...
    addExercise: (name, category) => { const e = { id: ids(), name: englishName(name), category }; setData(d => ({ ...d, exercises: [...d.exercises, e].sort((a,b) => a.name.localeCompare(b.name)) })); return e; },
    ...
    importCsv: csv => {
      ...
      setData(current => {
        ...
        for (const row of parsed.data) {
          ...
          const date = row.Date.trim(), name = englishName(row.Exercise), category = row.Category?.trim() || 'Other';
